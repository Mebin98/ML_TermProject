...
def load_data(user_data_file, spotify_data_file):
    """
    Load user and Spotify data, and prepare the filtered user dataframe.
    ...
    Returns:
    pandas.DataFrame: Filtered user data.
    """
    ...
    filtered_user_df = user[user['user_id'].isin(users_to_include['user_id'])]
...
def create_ratings_matrix(user_data):
    ...
    Args:
    user_data (pandas.DataFrame): Filtered user data containing user-item ratings.
...
def split_train_test_data(user_data, test_size=0.2, random_state=42):
    ...
    Args:
    user_data (pandas.DataFrame): Filtered user data.
...
def main(user_data_file, spotify_data_file):
    ...
    user_data = load_data(user_data_file, spotify_data_file)
